...
[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=clean-arch-aws-lambda-typescript&metric=security_rating)](https://sonarcloud.io/summary/new_code?id=clean-arch-aws-lambda-typescript)
...
>
> `destroy.yml` (also `workflow_dispatch`-only) runs `serverless remove` for a chosen `stage`/`region`, gated behind the same per-stage Environment and an explicit `confirm: DELETE` input to guard against accidental stack removal — mirrors the Azure/Spring destroy workflows.
